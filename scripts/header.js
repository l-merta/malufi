document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleScroll(headerElement, colorChange) {
    ...
    */
    ...
    if (
      (window.scrollY >= colorChange[0].top &&
        window.scrollY <= colorChange[0].height) ||
      (window.scrollY >= colorChange[1].top &&
        window.scrollY <= colorChange[1].height) ||
      (window.scrollY >= colorChange[2].top &&
        window.scrollY <= colorChange[2].height)
    ) {
      headerElement.classList = "header-dark";
    } else {
      headerElement.classList = "header-light";
